...
async fn outbox_publishes_entity_append_for_fact() {
    ...
    let result: Result<(), Box<dyn std::error::Error>> = async {
        ...
        let ce = tokio::time::timeout(Duration::from_secs(3), rx.recv()).await??;
...
async fn outbox_publishes_fact_then_goal() {
    ...
    let result: Result<(), Box<dyn std::error::Error>> = async {
        ...
        let fact_ce = tokio::time::timeout(Duration::from_secs(3), rx.recv()).await??;
        ...
        let goal_ce = tokio::time::timeout(Duration::from_secs(3), rx.recv()).await??;
