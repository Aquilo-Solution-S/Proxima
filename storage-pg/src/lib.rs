...
impl Storage for PgStorage {
    ...
    async fn subscribe_changes(
    ...
    ) -> Result<proxima_core::verbs::subscribe::ChangeEventStream, StorageError> {
        ...
        let owner_kind: &str = match &owner.principal {
            proxima_core::Principal::User(_) => "User",
            proxima_core::Principal::Group(_) => "Group",
        };
        ...
        for (seq,) in rows {
            if let Some(ce) = outbox::hydrate_change_event(&self.pool, seq).await? {
                backfill.push(ce);
            }
        }
    ...
    async fn query_memories(
        ...
        use proxima_core::verbs::query::{
            EntityKind, MemoryRow, QueryResponse, SupersessionStatus,
        };
        use proxima_core::{GroupId, MemoryId, OrgId, SchemaId, SchemaVersion, UserId};
        ...
        let seq_high_water =
            read_seq_high_water(&self.pool, owner_kind, owner_principal_id, owner_org_id).await?;
