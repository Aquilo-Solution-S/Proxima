...
- The schema is the migrated database. Dump it on demand with
  `scripts/regen-schema-sql.sh <dir>` (core vs code, from empty; do not
  commit the output). Stamp safety is `ensure_core_schema_markers`, not a
  committed dump.
