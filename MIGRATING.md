...
Every tool in `tools/list` now carries an `outputSchema` alongside its
...
### `Tool::Output` and `McpTool::Output` must derive `JsonSchema`

The bound on both traits gained one term:
