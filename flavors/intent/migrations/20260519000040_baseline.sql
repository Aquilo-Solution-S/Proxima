CREATE SCHEMA IF NOT EXISTS proxima_intent;
    ...
    IF NOT EXISTS (
        SELECT 1
         ...
         WHERE n.nspname = 'proxima_intent'
           AND t.typname = 'vision_ambition_level'
    ) THEN
...
CREATE TABLE IF NOT EXISTS proxima_intent.vision_brief_v1 (
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'vision_brief_v1_pkey'
           AND conrelid = 'proxima_intent.vision_brief_v1'::regclass
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_vision_brief_v1_goal
    ON proxima_intent.vision_brief_v1 USING btree (goal_id);

CREATE INDEX IF NOT EXISTS idx_vision_brief_v1_goal_activated_memory
    ON proxima_intent.vision_brief_v1 USING btree (goal_activated_memory_id);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'vision_brief_v1_memory_id_fkey'
           AND conrelid = 'proxima_intent.vision_brief_v1'::regclass
    ) THEN
        ALTER TABLE ONLY proxima_intent.vision_brief_v1
    ...
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'vision_brief_v1_goal_id_fkey'
           AND conrelid = 'proxima_intent.vision_brief_v1'::regclass
    ) THEN
        ALTER TABLE ONLY proxima_intent.vision_brief_v1
    ...
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conname = 'vision_brief_v1_goal_activated_memory_id_fkey'
           AND conrelid = 'proxima_intent.vision_brief_v1'::regclass
    ) THEN
        ALTER TABLE ONLY proxima_intent.vision_brief_v1
