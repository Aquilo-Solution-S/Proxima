...
fn substrate_schemas_register() {
    ...
    let relation_ids: HashSet<_> = frozen
        ...
        .collect();
    assert!(relation_ids.contains("proxima-agent-memory/agent-link-refers-to"));
    assert!(relation_ids.contains(CORE_DERIVED_FROM_RELATION));

    let resolved = frozen
        .resolve_relation("proxima-agent-memory/agent-link-refers-to")
        .expect("relation resolves");
    assert_eq!(
        resolved.payload_sidecar_table,
        Some("proxima_agent_memory.agent_link_v1")
    );
}
