...
-- What the code flavor's migrations add on top of core: the
-- proxima_code schema only. Foreign keys into proxima_core are the
-- flavor's dependency on core, stated explicitly. CI enforces that
-- a replay reproduces this file. See docs/how-to/migrations.md.
--
...
CREATE TABLE proxima_code.test_requested_v1 (
    ...
    CONSTRAINT test_requested_v1_depends_on_chk CHECK ((array_position(depends_on_memory_ids, NULL::uuid) IS NULL)),
...
CREATE TABLE proxima_code.work_assignment_v1 (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT work_assignment_v1_reason_chk CHECK (((char_length(reason) >= 1) AND (char_length(reason) <= 4000)))
);
...
CREATE TABLE proxima_code.work_requested_v1 (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    depends_on_memory_ids uuid[] DEFAULT ARRAY[]::uuid[] NOT NULL,
    CONSTRAINT work_requested_v1_depends_on_chk CHECK ((array_position(depends_on_memory_ids, NULL::uuid) IS NULL)),
