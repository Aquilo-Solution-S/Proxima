...
/// ```
///
/// Bundled recipes (Goose YAML shipped with the flavor crate):
///
/// ```ignore
/// proxima_flavor! {
///     name = "proxima-code",
...
/// ```
///
/// Each filename `foo` resolves to `<recipes_root>/recipes/foo.yaml` and is
/// recorded in the registry under the slug `<name>/foo`. WakeEntries
/// reference it as `recipe_ref = "bundled:<name>/foo"`.
#[macro_export]
            ...
            $($(
                {
                    let recipes_root: ::std::path::PathBuf =
                        ::std::path::PathBuf::from($recipes_root);
                    let filename: &str = $recipe_filename;
                    assert!(
                        !filename.is_empty(),
                        "recipe filename must be non-empty"
                    );
                    assert!(
                        !filename.contains('/') && !filename.contains('\\'),
                        "recipe filename {:?} must be a bare filename, not a path",
                        filename,
                    );
                    let slug = ::std::format!("{}/{}", $name, filename);
                    let path = recipes_root
                        .join("recipes")
                        .join(::std::format!("{}.yaml", filename));
                    registry.add_bundled_recipe(slug, path);
                }
            )*)?
        }
    };
