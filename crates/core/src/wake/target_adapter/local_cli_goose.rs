...
impl TargetAdapter for LocalCliGooseAdapter {
    async fn run(&self, invocation: TargetInvocation) -> Result<TargetOutcome, TargetAdapterError> {
        ...
        cmd.arg("--max-turns").arg(invocation.max_rounds.to_string());
        ...
        let stderr_tail = tail_lines(&stderr_full, 40);
        let turn_count = parse_turn_count(&stderr_full)
            .or_else(|| parse_turn_count(&String::from_utf8_lossy(&output.stdout)));
