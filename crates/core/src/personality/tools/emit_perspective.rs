...
pub struct EmitPerspectiveTool;
...
impl PersonalityTool for EmitPerspectiveTool {
    ...
    fn description(&self) -> &'static str {
        "Emit one Perspective memory. Provenance and wake_chain_depth are \
         auto-wired; the prior personality head, if any, is superseded \
         atomically."
    }
    ...
    async fn invoke(
        ...
        let mut parsed: EmitPerspectiveArgs = match serde_json::from_value(args) {
            Ok(v) => v,
            Err(e) => {
        ...
        let handle = ctx.handles.assign_perspective_memory(memory_id);
