...
async fn discovery_to_mutation_smoke() -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn call_tool(
        ...
        let resp = post_rpc(client, url, Some(session), bearer,
            json!({
                "jsonrpc": "2.0",
                "id": 1,
                "method": "tools/call",
    ...
    let names: std::collections::HashSet<_> = arr
        ...
        .collect();
    assert!(names.contains("core/list_personalities"),
        "MCP CRUD tool present in discovery output");
    assert!(names.contains("core/instantiate_personality"),
        "MCP CRUD tool present in discovery output");
    ...
    assert!(items.iter().any(|p| p["display_name"].as_str() == Some("TestSubject")),
