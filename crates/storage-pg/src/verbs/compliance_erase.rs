...
/// The key material one Fact ingest is admitted against.
#[derive(Debug, Clone, Copy)]
pub struct FactSuppressionProbe<'a> {
    pub owner: OwnerRef,
...
/// Admit a whole batch of Fact ingests in one probe.
///
/// The set-based twin of [`check_suppression_for_fact_tx`], with the same
/// three key classes per unit: nothing is hoisted out of the per-unit key
/// material — the owner-scope class varies with the unit's source, the
/// batch class with its source batch, and the content class with its
/// receipt — so the batch carries every unit's keys and asks once.
///
/// The batch is one transaction, so one suppressed unit refuses the whole
/// batch, exactly as one suppressed unit refuses its own transaction on the
/// per-unit path.
///
/// # Errors
///
/// Returns [`StorageError::Suppressed`] when any probe's owner/source,
/// source batch, or receipt content is under a compliance suppression key.
pub async fn check_suppression_for_facts_tx(
    tx: &mut Tx<'_>,
    probes: &[FactSuppressionProbe<'_>],
) -> Result<(), StorageError> {
    if probes.is_empty() {
        return Ok(());
    }
    let mut owner_kinds = Vec::with_capacity(probes.len());
    let mut owner_keys = Vec::with_capacity(probes.len());
    let mut owner_ids = Vec::with_capacity(probes.len());
    let mut source_ids = Vec::with_capacity(probes.len());
    let mut source_batch_ids = Vec::with_capacity(probes.len());
    let mut receipt_ids = Vec::with_capacity(probes.len());
    for probe in probes {
        let (owner_kind, owner_id) = suppression_owner_parts(probe.owner);
        owner_kinds.push(owner_kind);
        owner_keys.push(probe.owner.stable_key_uuid().to_string());
        owner_ids.push(owner_id);
        source_ids.push(probe.source_id.as_str());
        source_batch_ids.push(probe.source_batch_id);
        receipt_ids.push(probe.receipt_id);
    }

    let exists: Option<(i32,)> = sqlx::query_as(
        "WITH unit(owner_kind, owner_key, owner_id, source_id,
                   source_batch_id, receipt_id) AS (
             SELECT * FROM unnest($2::text[], $3::text[], $4::text[],
                                  $5::text[], $6::uuid[], $7::bytea[])
         ),
         candidate_keys(key_class, suppression_key) AS (
             SELECT 'owner_source_scope'::proxima_core.compliance_suppression_key_class,
                    decode(md5($1 || chr(31) || 'owner_source_scope' || chr(31) || u.owner_kind || chr(31) || u.owner_key || chr(31) || coalesce(u.owner_id, '') || chr(31) || u.source_id), 'hex')
               FROM unit u
             UNION ALL
             SELECT 'source_batch'::proxima_core.compliance_suppression_key_class,
                    decode(md5($1 || chr(31) || 'source_batch' || chr(31) || u.owner_kind || chr(31) || u.owner_key || chr(31) || coalesce(u.owner_id, '') || chr(31) || u.source_batch_id::text), 'hex')
               FROM unit u
             UNION ALL
             SELECT 'receipt_content'::proxima_core.compliance_suppression_key_class,
                    decode(md5($1 || chr(31) || 'receipt_content' || chr(31) || u.owner_kind || chr(31) || u.owner_key || chr(31) || coalesce(u.owner_id, '') || chr(31) || encode(u.receipt_id, 'hex')), 'hex')
               FROM unit u
         )
         SELECT 1
           FROM proxima_core.compliance_suppression_keys s
