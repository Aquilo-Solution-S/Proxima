...
async fn core_closed_vocab_columns_use_sql_enums() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = async {
        ...
        for (table, column) in [
            ...
            ("personality_wake_entries", "execution_mode"),
            ("personality_wake_entries", "authored_by"),
            ("personality_wake_entries", "model_tier"),
            ("personality_wake_entries", "goal_scope"),
            ("personality_wake_invocations", "status"),
            ("inference_targets", "kind"),
            ("inference_tier_bindings", "tier"),
        ] {
