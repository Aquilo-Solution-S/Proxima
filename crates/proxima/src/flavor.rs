...
/// transaction begins. A text the provider refuses whole is embedded in
/// pieces and stored as one chunked version with the row. A text refused
/// at every length leaves the memory written with no vector and an
/// embedding job enqueued in the same transaction, and
