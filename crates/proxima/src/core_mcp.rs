...
impl CoreMcpTools {
    ...
    /// Dispatch `core/search_memories` through the embedded MCP facade.
    ///
    /// # Errors
    ///
    /// Returns authorization, registry lookup, or tool execution errors.
    pub async fn search_memories(
        ...
        self.call_core_tool(authz, owner, model_id, "core/search_memories", args)
            .await
    }

    /// Dispatch `core/fetch_memory` through the embedded MCP facade.
    ///
    /// # Errors
    ///
    /// Returns authorization, registry lookup, or tool execution errors.
    pub async fn fetch_memory(
        ...
        self.call_core_tool(authz, owner, model_id, "core/fetch_memory", args)
            .await
    }

    /// Dispatch `core/facts_citing_object` through the embedded MCP facade.
    ///
    /// # Errors
    ///
    /// Returns authorization, registry lookup, or tool execution errors.
    pub async fn facts_citing_object(
        ...
        self.call_core_tool(authz, owner, model_id, "core/facts_citing_object", args)
            .await
    }

    /// Dispatch `core/citation_of_fact` through the embedded MCP facade.
    ///
    /// # Errors
    ///
    /// Returns authorization, registry lookup, or tool execution errors.
    pub async fn citation_of_fact(
        &self,
        authz: AuthzContext,
        owner: Owner,
        model_id: Option<String>,
        ...
        self.call_core_tool(authz, owner, model_id, "core/citation_of_fact", args)
            .await
    }
}
