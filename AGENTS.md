...
| Tier | Tools |
|---|---|
| Substrate (always) | 9 tools + 7 resources across memory, goals, personality, wake-config, read-scope, citations, fact tombstones, and introspection. `proxima://tools` returns the live tool catalog only; resources are discovered via MCP `resources/list` and `resources/templates/list`. Key writes: `core_remember`, `core_record_utterance`, `core_derive`, `core_link`, `core_goal` (`{"action":"set", ...}`). |
