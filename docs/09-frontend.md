...
`Goal`) is `serde_json` of the flavor's payload struct, opaque to
the wire. The split keeps source-of-truth singular: payload
schemas are declared as Rust traits per
...
Payloads are `serde_json::to_vec` on send, `JSON.parse(...)` after
`TextDecoder().decode(...)` on receive. The TS client dispatches on
`(schema_id, schema_version)` to the typed decoder:
...
export const PAYLOAD_DECODERS = {
  "proxima-code/commit-v1@1": (b: Uint8Array) =>
    JSON.parse(new TextDecoder().decode(b)) as CommitV1,
  // ...
