...
| Protocol constants | duplicated tool/resource strings do not drift | `proxima_core::protocol` constants consumed by MCP/selfdoc/profile code | core/MCP tests; `scripts/check-architecture-guardrails.py` stale production-code tombstone/source-identity checks |
...
| Raw storage denial | flavors cannot depend on stable raw core DB access | no stable `PgPool`/aggregate `Storage`; flavor raw `proxima_core.*` SQL denied | `scripts/check-architecture-guardrails.py`; `crates/proxima/tests/public_api_tiers.rs` |
