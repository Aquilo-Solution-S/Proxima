...
| Protocol constants | duplicated tool/resource strings do not drift | `proxima_core::protocol` constants consumed by MCP/selfdoc/profile code | core/MCP tests; `scripts/check-v004-architecture-ratchets.py` stale production-code tombstone/source-identity checks |
...
| Raw storage denial | flavors cannot depend on stable raw core DB access | no stable `PgPool`/aggregate `Storage`; flavor raw `proxima_core.*` SQL denied | `scripts/check-v004-architecture-ratchets.py`; `crates/proxima/tests/public_api_tiers.rs` |
