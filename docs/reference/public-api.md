...
| bound | candidate lists are capped (2,000 ids) and deduplicated before they ever reach a query, so a pathological caller cannot force an unbounded `IN (...)`/`ANY($1)` scan |
