...
| Host API (system work) | `AuthzContext::for_system(&SystemAuthority, OwnerRoles)` | sealed `AuthPath::System` context with no bearer; the runtime's `SystemAuthority` is the only mint |
