...
| `PROXIMA_PG_HNSW_MAX_SCAN_TUPLES` | Postgres search | `20000` | bounding iterative scans | only sent when it differs from pgvector's default and iterative scan is on; range `1..=2147483647` (the GUC's own bounds); out-of-range refuses at boot |
