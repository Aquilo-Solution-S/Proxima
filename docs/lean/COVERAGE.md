...
| PO-4 | Replay cannot recreate revoked origin; existing live `t` and hard-delete witness prevent fresh recapture; a fresh distinct `t` can reuse the same SourceId | `replay_does_not_restore_revoked_origin`, `live_fact_cannot_be_freshly_recaptured`, `hard_deleted_fact_cannot_be_freshly_recaptured`, `fresh_fact_can_reuse_revoked_source` |
