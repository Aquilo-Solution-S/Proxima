...
| OB-4 | Delivery state is a closed vocabulary `{pending, claimed(lease), published}` with a closed admitted transition set | STRUCTURAL: `inductive Delivery` (3 constructors, lease carried inside `.claimed`) + `inductive Step` (4 constructors: claim / ack / release / expire; none departs `.published`, none deletes) — the `goalTransitionAdmitted` pattern (GO-1/GO-2). `Step` relates `Delivery`, not `Record`, so state moves only through `advance` |
...
| OB-6 | Owner erasure is the only removal path; the outbox is not an untracked copy that outlives its owner | THEOREM `erasure_is_the_only_removal` over `outboxErase`, reusing `abandoned` (CO-7 / ST-13) — NOT `wipeable`: a publication record has no cooled/unreferenced analogue, so that disjunct has nothing to bind. Storage side is a `Cascade { via }` declaration on the Fact (doc 13 §What an erase destroys) |
