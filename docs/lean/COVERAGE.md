...
| U-4 | Reality enters as typed Facts | `Fact` subtype (`Memory` with kind `.Fact`) + `operator_memory_output_not_fact`; `Flavor.OptionalFactReceipt` witnesses optional event/receipt metadata without certifying external truth; the outbound half (a declared Fact's publication record) is OB-1 |
...
| SR-2 | Every memory payload schema-typed | CORRECTED 2026-09-10: this cell named `Memory.schema`, a field that no longer exists — `schema` moved to the series catalog and lives only on `MemoryHead` (see SR-49). Live carrier: structure field `MemoryHead.schema : SchemaRef` + accessor `memory_head_schema`, with `MemoryHeadAligned` binding every row to its head, so schema-typing is a series property (accessor totality — every series carries an opaque schema tag). Schema registration in the active registry remains engine admission, not yet a kernel rule (D16) |
...
Carriers live in `Causa/Publication.lean` (doc [18](../18-fact-outbox.md)).

| ID | Invariant | Carrier |
|---|---|---|
| OB-1 | An admitted Fact has a publication record iff its series is listenable; a non-listenable Fact has none | ASSERTED `OutboxValid.captureTotal` / `captureSound` (table validity — the kernel cannot observe a write); projection THEOREM `capture_iff_listenable`. Listenability is read off the series (`factListenable` over `MemoryHead`, SR-49), never off the version row, and the registry is a PARAMETER (`ListenRegistry`), never a kernel table — same openness stance as CF-OPEN |
| OB-2 | One record per Fact `t`; identity is the immutable Fact row id, not the handle, receipt or content hash; a replayed admission creates no second record | ASSERTED `OutboxValid.idsUnique` (`RecordIdUnique`); THEOREMs `replay_no_second_record`, `record_id_is_fact_t`. Replay reuses the same `t`, so it names the same record by construction (ST-22/23 identity rule) |
| OB-3 | The captured half is immutable: the publisher sends the bytes captured at admission, never a re-derivation from today's Fact state or an upgraded flavor | STRUCTURAL: `Record = Captured × Delivery`, and no step touches `Captured` — `advance` only rewrites `delivery`. THEOREM `step_preserves_capture`; `instance : Immutable Captured` (ST-11 / CO-1 class). ASSERTED `OutboxValid.captureFaithful` pins owner and original recording tick to the Fact |
| OB-4 | Delivery state is a closed vocabulary `{pending, claimed(lease), published(publishedAt)}` with a closed admitted transition set | STRUCTURAL: `inductive Delivery` (3 constructors; the lease is carried inside `.claimed` and the acknowledgement tick inside `.published`, so no `Record` proof field is needed for either) + `inductive Step` (4 constructors: claim / ack / release / expire; none departs `.published`, none deletes) — the `goalTransitionAdmitted` pattern (GO-1/GO-2). `Step` relates `Delivery`, not `Record`, so state moves only through `advance`. `.published` carries its tick because OB-9's horizon is stated against the DELIVERY time, not the capture time |
| OB-5 | No transition removes a record; an expired lease returns it to `pending` with its bytes intact — expiry permits retry, never loss | THEOREMs `no_step_deletes`, `expiry_preserves_record` over `outboxStep` / `advance` |
| OB-6 | Owner erasure is the only removal path that can reach a record in any delivery state; the outbox is not an untracked copy that outlives its owner | THEOREM `erasure_is_the_only_removal` over `outboxErase`, reusing `abandoned` (CO-7 / ST-13) — NOT `wipeable`: a publication record has no cooled/unreferenced analogue, so that disjunct has nothing to bind. Storage side is a `Cascade { via }` declaration on the Fact (doc 13 §What an erase destroys). NARROWED by OB-9: retention is a second removal, bounded to DELIVERED records, so "the only removal" now reads "the only removal of an undelivered record" — proved, not merely asserted, by `prune_never_removes_undelivered` |
| OB-7 | A record carries its Fact's owner; the outbox opens no second, cross-owner read path | THEOREM `owner_follows_fact` — definitional (`rfl` + `Iff.rfl` against `may_read`), so a record's readability IS its Fact's readability; ASSERTED `OutboxValid.captureFaithful` for stored rows. Producer identity is a `User` bound at capture (configured credentials), never a self-asserted message header |
...
| ID | Invariant | Carrier |
|---|---|---|
| HL-1 | Lifecycle requests bind a typed `OwnerRef` to whole-owner, typed-source, or distinct exact-Facts scope; requests and receipts carry separate typed physical `MemoryId` and original-copy `(OwnerRef, MemoryId)` selection sets | `LifecycleRequest`, `Scope`, `SourceScopeId`, shared `FactCopyLocator` / `CopyEraseSelection`; `exact_facts_scope_is_distinct`, `committed_erase_binds_owner_and_scope` |
| HL-2 | Frozen participant tables have complete, duplicate-free erase/export policies—including an explicit exact-Fact disposition—and are disjoint from generic-path tables | `CoverageComplete`, `ExactTableSet`, `SurfacePolicy.exactFactErase`; `coverage_is_complete_and_disjoint`; `example_registration_has_complete_disjoint_coverage` |
| HL-3 | Erase receipts bind participant, owner, scope and both typed selection sets; mismatched selection invalidates the receipt. Counts cover exact declared table keys with separate deleted/scrubbed values; retained policies report zero counts for source and exact-Facts scopes; exact-Facts selections carry no original-copy locators | `EraseReceiptValid`, `selectionMatches`, `receipt_binds_participant_owner_scope_and_tables`, `receipt_binds_typed_selection`, `selection_mismatch_invalidates_receipt`, `exact_facts_request_and_receipt_have_no_original_copy_selection`, `retained_source_surface_has_zero_counts`, `retained_exact_fact_surface_has_zero_counts` |
| HL-4 | Missing registration/callback, incomplete coverage, callback failure, invalid receipt or failed commit rejects the erase and preserves core plus host state | Two directions, both needed. "Preserves": `rejected_erase_preserves_core_and_host` maps `.rejected` to the identical input state — true by `stateAfterErase`, and it does NOT establish that any bad input rejects. "Rejects": `committed_erase_requires_every_admission_condition` inverts a committed result back onto all five conditions, with `invalid_receipt_never_commits`, `failed_commit_never_commits`, `missing_registration_never_commits` and `incomplete_coverage_never_commits` as the per-clause prohibitions |
| HL-5 | Commit requires complete coverage, a valid same-owner/scope/selection receipt and successful commit; the core candidate remains supplied by the existing authorized erasure path | `EraseTransaction.committed`, `committed_erase_binds_owner_and_scope`; no erase capability or Compliance rule is added |
...
| HL-R | Boot capture, existing erasure authorization and selector truth, SQL transaction isolation, callback SQL fidelity and row-count truth; commit observation | EXCLUDED: runtime evidence required; the model does not prove supplied physical/original-copy selections match database rows or refine arbitrary SQL. Tests cover task cancellation before COMMIT and a server-rejected COMMIT. A connection loss or cancellation after COMMIT dispatch can leave the caller unable to observe the database's atomic outcome; the model assumes known committed-vs-rejected state and does not prove transport outcome reporting. |
...
| HWA-R | Existing authz admission and context authenticity remain trusted runtime boundaries | `subjectless_denied_ordinary_host_write_never_dispatches` verifies denied `AuthzContext::denied_for_owner` has zero participant calls. The Lean model starts after ordinary authorization; no new `may_write` rule, role, cognitive permit, or proof of caller authentication is claimed. Nor is the §21 Lean load-bearing on its own: `stampPermit` and `stampOrdinaryPermit` discard `_targetOwner` and `_command` by construction, so every theorem above closes by `rfl`. They pin the SIGNATURE — an edit making attribution depend on target or payload breaks the build — and the runtime carriers are what actually defend the property |

## 22 — Publication origin (PO)

Carriers live in `Causa/PublicationOrigin.lean`.

| ID | Invariant | Carrier |
|---|---|---|
| PO-1 | Fresh admission adds a payload-free origin keyed by Fact `t` atomically with the Fact/outbox key, only when the `t` is absent and has no hard-delete witness; key uniqueness is preserved | `PublicationStep.freshAdmission`, `FreshCaptureAllowed`, `fresh_admission_captures_atomically`, `fresh_capture_preserves_key_uniqueness` |
| PO-2 | Transfer and outbox pruning preserve original typed owner/source; source revocation removes matching origin, outbox ID and captured-owner projection while retaining live Fact/current owner; full-owner revocation removes every matching original-owner origin/copy, including source=None, and preserves unrelated owners/IDs | `transfer_preserves_original_owner_and_source`, `outbox_pruning_preserves_original_owner_and_source`, `source_revoke_removes_matching_origin`, `source_revoke_preserves_nonmatching_origin`, `source_revoke_removes_matching_outbox_copy`, `source_revoke_removes_captured_outbox_owner`, `source_revoke_preserves_unmatched_outbox_id`, `source_revoke_retains_live_facts_and_current_owners`, `owner_revoke_removes_matching_origin`, `owner_revoke_preserves_other_owner_origin`, `owner_revoke_removes_matching_outbox_copy`, `owner_revoke_removes_captured_outbox_owner`, `owner_revoke_removes_source_free_outbox_copy`, `owner_revoke_preserves_unmatched_outbox_id`, `owner_revoke_retains_live_facts_and_current_owners` |
| PO-3 | Intake payload handling requires the matching original owner and origin, plus absence of a core hard-delete witness; missing origins and witnesses fail closed; backlog re-offer uses the same gate | `eligibleForPayload`, `intake_eligibility_requires_matching_origin_and_no_witness`, `missing_origin_fails_closed`, `hard_delete_witness_fails_closed`, `backlog_reoffer_uses_intake_eligibility` |
| PO-4 | Replay cannot recreate revoked origin; existing live `t` and hard-delete witness prevent fresh recapture; a fresh distinct `t` can reuse the same SourceId | `no_step_restores_revoked_origin` carries the first clause as a universal over every step; `replay_does_not_restore_revoked_origin` is the weaker existential it generalizes and is NOT on its own evidence for "cannot". `live_fact_cannot_be_freshly_recaptured`, `hard_deleted_fact_cannot_be_freshly_recaptured` discharge its `blocked` hypothesis for the two runtime bars; `fresh_fact_can_reuse_revoked_source` |
| PO-5 | Exact-`t` hard erase removes matching origin/outbox irrespective of original owner and preserves unrelated `t` origins; this is an operation law, not proof every runtime call site invokes it | `exact_fact_revoke_is_global`, `exact_fact_revoke_preserves_other_fact` |
| PO-6 | Backfill requires surviving outbox + retained Fact evidence, no hard-delete witness, and explicit typed owner/source metadata; missing or pruned history fails closed | `BackfillEvidence`, `backfill_requires_surviving_evidence`, `missing_backfill_evidence_fails_closed`, `missing_owner_or_source_metadata_fails_closed`, `backfill_accepts_known_source_absence` (a worked instance over `emptyState` with one capture, not a general law), `missing_known_source_evidence_fails_closed` |
| PO-7 | Backfill binds the outbox Fact to its original typed owner in the model, so transfer cannot retarget old publication provenance | `CapturedOutboxOwner`, `fresh_capture_stamps_outbox_original_owner`, `transfer_preserves_captured_outbox_owner`, `CapturedOutboxOwnerUnique`, `backfill_after_transfer_uses_captured_original_owner` (likewise an `emptyState` instance, not a general law); runtime recovers owner kind through existing outbox `owner_id` joined to retained `owners.kind`, with source evidence from retained hot/cooled Fact rows. Raw SQL mutation of the owner-kind pair remains a trusted-host residual. |
| PO-R | Atomic database admission/erase, existing core witness creation, real source attribution, migration ordering, single-target custom transaction wiring, and actual intake/publisher call sites | EXCLUDED: host runtime and migration evidence required; no SQL locks, transactions, full Fact reads, tombstones, or external broker rollback are modeled |

## 23 — Host-state Fact-copy inverse (HFC)

Carriers live in `Causa/HostStateFactCopyErase.lean`.
`Causa/HostStateFactCopyEraseAudit.lean` checks their axiom surface.

| ID | Invariant | Carrier |
|---|---|---|
| HFC-1 | Physical selected Fact IDs and original-publication copy locators `(OwnerRef, MemoryId)` are distinct shared request/receipt selection domains | `HostStateLifecycle.CopyEraseSelection` / `FactCopyLocator`, request/receipt `selectionMatches`; `exactPhysicalFact`, `byOriginalScope`, `exact_fact_selection_has_no_original_scope` |
| HFC-2 | Exact physical Fact erasure removes that Fact's copies across original owners; combined OR-selection removes rows selected by either or both domains and preserves rows selected by neither | `physical_selection_crosses_original_owners`, `exact_physical_fact_removes_its_copies_globally`, `lifecycle_selection_removes_physical_copy`, `lifecycle_selection_contains_intersection`, `lifecycle_selection_removes_intersection_copy`, `combined_selection_preserves_unselected_copy`, `exact_physical_fact_preserves_other_fact` |
| HFC-3 | Source selection uses immutable original owner+source evidence; owner selection includes source-free origins; selected rows are removed by the combined selection and unselected copies remain | `originalCopyLocators`, `source_copy_locators_are_exact`, `owner_copy_locators_are_exact`, `lifecycle_selection_removes_original_scope_copy`, `owner_scope_selects_source_free_origin`, `original_owner_source_scope_preserves_unselected_copy`, `source_scope_does_not_select_known_absent_source` |
| HFC-4 | The inverse deletes copied payload rows without changing independently authored configuration | `eraseCopiedPayloads`, `inverse_preserves_authored_configuration` |
| HFC-5 | Lifecycle source and publication-origin source wrappers denote the same native Proxima source token; projection preserves tokens and is injective, and source-copy selection uses that projection | `publicationSourceForScope`, `publication_source_scope_preserves_token`, `publication_source_scope_injective`, `publication_source_scope_equal_iff_tokens_equal`, `mapped_source_copy_locators_are_exact`; Rust carries the existing typed `SourceId` into `HostStateEraseScope::Source` by clone. CloudEvents' configured producer URI is a separate identity and is not mapped here |
| HFC-R | Existing authorization/physical ID selection, origin-index selection, callback SQL fidelity, exact-once SQL row counts and transactional composition | EXCLUDED: selections are supplied by existing authorized core erase and retained provenance index; this model proves set-level union/filter behavior only, not arbitrary SQL, selector correctness or truthful/exact-once database counts |
