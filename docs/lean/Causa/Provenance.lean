...
def derivesFrom (registry : RelationRegistry) (m m' : Memory) : Prop :=
...
axiom derivation_created_at_strict :
  ∀ (registry : RelationRegistry) (m m' : Memory),
    derivesFrom registry m m' → memory_created_at m' < memory_created_at m
...
theorem grounding_wf (registry : RelationRegistry) :
    WellFounded (fun lo hi => derivesFrom registry hi lo) :=
  Subrelation.wf
    (fun {a b} h => derivation_created_at_strict registry b a h)
    (invImage memory_created_at Nat.lt_wfRel).wf

/-- A memory is grounded when repeated descent reaches Facts. -/
inductive GroundsInFact (registry : RelationRegistry) : Memory → Prop where
  | fact {m} : memory_kind m = .Fact → GroundsInFact registry m
  | step {m m'} : derivesFrom registry m m' →
      GroundsInFact registry m' → GroundsInFact registry m

/-- N1 bottoms-out theorem: well-founded descent plus per-derived-row
    provenance entails Fact grounding for every memory. -/
theorem memory_grounds_in_facts (registry : RelationRegistry) :
    ∀ m : Memory, GroundsInFact registry m := by
  intro m
  exact (grounding_wf registry).induction m
    (fun m ih => by
      by_cases hfact : memory_kind m = .Fact
      · exact GroundsInFact.fact hfact
      · obtain ⟨e, hc, hs, ⟨mt, ht⟩⟩ := derived_has_provenance registry m hfact
        have hder : derivesFrom registry m mt := by
...
theorem abstraction_grounds_in_facts :
    ∀ registry m, memory_kind m = .Abstraction → GroundsInFact registry m :=
  fun registry m _ => memory_grounds_in_facts registry m
