...
| MCP auth rejected | bearer token or owner selection mismatch | use `Authorization: Bearer pxm_<master-token>` and `X-Proxima-Owner: personal:<user-id>` for dev server |
