...
| `pg_trgm` | no | Trigram GIN indexes can serve `LIKE '%…%'` substring predicates. Since v0.0.8 the ranked arms scan `<flavor>.projection` and the substring arm is a separate statement against the sidecars, with no index of its own. That is a **cost** decision, not an impossibility: the predicate is `lower(concat_ws(…))` over a schema's declared search fields, and every shipped sidecar already carries a `STORED GENERATED embed_text` column built from the same fields — proof that a migration can name such an expression, and a column a `gin (… gin_trgm_ops)` index could name directly. None ships because the arm is already narrow: it is opt-in per schema (`SubstringArm::Off` is the default) and runs only over the schemas whose ranked arm returned zero rows, so an always-on write cost would buy an index most statements never reach. Measure before adding one — every write pays for it. |
...
| `PROXIMA_OIDC_JWKS_JSON` | no | `{"keys":[{"kty":"RSA","kid":"k1","n":"...","e":"AQAB","alg":"RS256","use":"sig"}]}` | Pinned JWKS for a host with no route to its issuer: nothing is fetched, the issuer is only matched against `iss`. Mutually exclusive with `PROXIMA_OIDC_JWKS_URI`; see [10](10-configuration.md#mcp-endpoint-and-authentication). |
...
| JetStream storage | **DevOps-owned.** Configure the stream's storage and persistent volume according to the deployment's durability requirement. Proxima receives only the `PubAck` that the broker accepted the bytes |
| topology | **DevOps-owned.** Provision streams, subject transforms, partitions and durable consumers before enabling the publisher. Test the complete source → transform → consumer path separately |
| capacity | **DevOps-owned** for the broker; `discard: New` (or the chosen deployment semantics) determines whether a full stream rejects a publish. `PROXIMA_OUTBOX_MAX_PENDING` remains the database-side bound and fails listenable writes with `CapacityExhausted` (HTTP **503**) |
| freeing stream space | an ACK does **not** necessarily free it: the deployment's retention policy decides. For `Limits` retention, purge only sequences at or below every consumer's `ack_floor` ([18 §Stream space never frees itself](18-fact-outbox.md#stream-space-never-frees-itself)) |
| existing stream | Proxima neither creates nor validates it. The publisher must work with credentials that have publish and reply-inbox permissions but no stream-management permissions |
