...
- Inline embedding is one provider request per text. Bulk writers call `.embedding_mode(EmbeddingMode::Deferred)`: the row lands with a pending job (the Fact lane), and the embedding drain sends queued texts `PROXIMA_EMBED_BATCH_SIZE` per request. Semantic search finds the row once drained; a provider outage leaves the job pending. Code-chunk ingest does this.
