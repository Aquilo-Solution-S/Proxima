...
| Host extra-table | `AppContext::{clone_pool_for_host, pg_tuning_for_host}` | wrap the pool + resolved query policy in a flavor store inside `FlavorApp::services`; tools never see the pool. Second connection — not atomic with a UnitOfWork Fact |
| Host-state in UnitOfWork | `UnitOfWork::apply_host_state`, `PgHostStateParticipant` | Host API: typed host-owned state on the existing write session; owner write-gate; tables must be declared `state_surfaces` |
...
Goals written before `0006` carry no declaration, and one is never guessed
back from current target rows. Reusing such an id is therefore an idempotency
conflict rather than a replay: a consumer that keeps stable idempotency keys
across an upgrade from v0.0.9 or earlier must retire the keys it wrote before
that upgrade.
